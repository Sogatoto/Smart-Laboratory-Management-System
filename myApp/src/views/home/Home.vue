...
import { AUDIT,APPROV,ClassType, StateType } from "../../util/type";
...
const addTextLayer = (data) => {
  ...
  bindTextLayerEvent(textlayer);
  scene.addLayer(textlayer);
};
const bindTextLayerEvent = (textlayer) => {
    popup && popup.close()
    textlayer.on("click", async(e) => {
        ...
        var res = await Promise.all(
            [   
                ...
                axios.get(`/adminapi/books?lab_id=${e.feature.id}&book_state=${APPROV}`)
            ]
        ) 
        ...
        <li class="home-item el-card is-always-shadow box-card">
            ...
            <span class="${item.book_state===0?'yellow':'green'}">${StateType.find(data=>data.value===item.book_state).label}</span>
        </li >
        `).join("")
        }else{
